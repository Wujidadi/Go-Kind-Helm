...
# 專案參數
REPOSITORY := wujidadi/go-app
CLUSTER_NAME := go-k8s
RELEASE_NAME := go-helm
CHART_NAME := go
...
# Ingress 參數
INGRESS_NAMESPACE := go-ingress
INGRESS_CLASS := go-ingress-nginx
HTTP_PORT := 30840
HTTPS_PORT := 30843

# PostgreSQL 參數
PG_HELM_VERSION := 16.5.5
PG_RELEASE_NAME := postgres-go-helm
...
REDIS_RELEASE_NAME := redis-go-helm
REDIS_CHART_NAME := redis
REDIS_SUB_NAME := master
REDIS_PORT := 6379
REDIS_HOST_PORT := 63790
REDIS_SECRET := redis-go-secret  # 要和 charts/redis/values-*.yaml 的 auth.existingSecret 相同
...
# 列出 Pod 的中容器名稱
get-containers:
	@PODS=$$(kubectl get pods -n go-helm -o jsonpath="{.items[*].metadata.name}"); \
	for POD in $$PODS; do \
		echo "🔎 Pod: $$POD"; \
		CONTAINERS=$$(kubectl get pod $$POD -n go-helm -o jsonpath="{.spec.containers[*].name}"); \
...
shell:
	...
	for POD in $$PODS; do \
		...
		for CONTAINER in $$CONTAINERS; do \
			...
			if kubectl exec -it $$POD -n go-helm -c $$CONTAINER -- /bin/zsh 2>/dev/null; then \
...
shell-pick:
	@bash -c '\
		POD_LIST=$$(kubectl get pods -n go-helm -l $(LABEL_KEY)=$(CHART_NAME) -o jsonpath="{.items[*].metadata.name}"); \
		PS3="請選擇要進入的 Pod："; \
		select SELECTED_POD in $$POD_LIST; do \
			[ -n "$$SELECTED_POD" ] && break; \
		done; \
		CONTAINERS=$$(kubectl get pod $$SELECTED_POD -n go-helm -o jsonpath="{.spec.containers[*].name}"); \
		...
		echo "👉 正在檢查可用 shell ..."; \
		SHELL_CMD=$$(kubectl exec $$SELECTED_POD -n go-helm -c $$SELECTED_CONTAINER -- sh -c "command -v zsh || command -v bash || command -v sh"); \
		if [ -z "$$SHELL_CMD" ]; then \
			read -p "找不到預設 shell，請自行輸入要執行的指令（例如 /bin/sh）： " SHELL_CMD; \
		fi; \
		CMD="kubectl exec -it $$SELECTED_POD -n go-helm -c $$SELECTED_CONTAINER -- $$SHELL_CMD"; \
		echo "👉 $$CMD"; \
		eval $$CMD \
	'
...
# 自動清理孤兒 namespaces
cleanup-namespaces:
	CMD="kubectl get ns | grep -E 'go|redis|postgres' | awk '{print \$$1}' | xargs -I {} kubectl delete ns {}"; \
	echo "👉 $$CMD"; \
	eval $$CMD
